...
from models.build_LwF import build_lwf_model
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_root', type=str, default='./outputs_LwF/')
    ...
    # Experimental Dir.
    model_dir = os.path.join(args.exp_root, f"{runner_name}_{args.model_name}_{args.dataset_name}_Steps{args.num_steps}_Wkd_{args.w_kd}")
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    args.log_dir = model_dir + f'/{runner_name}_{args.model_name}_{args.dataset_name}_Steps{args.num_steps}_Wkd_{args.w_kd}_log.txt'
    ...
    # WandB setting
    # if args.mode == 'train':
    #     wandb_run_name = f'LwF_{args.dataset_name}_{args.model_name}_{args.num_steps}-Steps_Wkd_{args.w_kd}'
    #     wandb.init(project='SOTA_LwF',
    #                entity=args.wandb_entity,
    #                tags=[f'TotalStep={args.num_steps}', args.dataset_name, args.model_name,
    #                      f'Wkd={args.w_kd}', f'device={args.device}'],
    #                name=wandb_run_name,
    ...
    # Model creation
    # ----------------------
    encoder, step_single_head_list, joint_head_container_list = build_lwf_model(args)
    ...
    if args.mode == 'train':
        ...
        for step_train in range(args.num_steps):
            wandb_run_name = f'LwF_{args.dataset_name}_{args.model_name}_{step_train}/{args.num_steps}-Steps_Wkd_{args.w_kd}'
            wandb_run_step = wandb.init(
                project='SOTA_LwF',
                entity=args.wandb_entity,
                tags=[f'TotalStep={args.num_steps}', args.dataset_name, args.model_name,
                      f'Wkd={args.w_kd}', f'device={args.device}', f'CurrentStep={step_train}'],
                name=wandb_run_name,
                mode=args.wandb_mode,
                reinit=True
            )
