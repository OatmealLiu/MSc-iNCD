...
def build_direct_concat_model(args):
    ...
    if args.current_step > 0:
        super_state_dict = torch.load(args.save_sup_model_path)
        model.load_state_dict(super_state_dict)
        model = model.to(args.device)
    ...
    temp_single_head = LinearHeadCosNorm(in_dim=feat_dim, out_dim=args.num_novel_per_step)

    temp_single_head = temp_single_head.to(args.device)
    ...
    return model, single_head, single_heads_list, joint_head, temp_single_head
