...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, no_class=10, in_channel=3, zero_init_residual=False):
        ...
        self.in_planes = 64    # 64,224
        # 64,224
        self.conv1 = nn.Conv2d(in_channel, 64, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(64)  # 64,224
...
def build_resnet18_plain(**kwargs):
...
def build_resnet50_plain(args):
    model = torch.hub.load('facebookresearch/dino:main', 'dino_resnet50')
    model.apply(weight_reinit)
    model = model.to(args.device)
    return model
