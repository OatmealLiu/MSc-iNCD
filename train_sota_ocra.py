...
if __name__ == "__main__":
    ...
    parser.add_argument("--w_entropy", default=3.0, type=float, help="weight for entropy loss")
    parser.add_argument('--rs_bce', action='store_true', default=False)
    ...
    model_dir = os.path.join(args.exp_root, f"{runner_name}_{args.dataset_name}_Steps{args.num_steps}_{args.model_head}_NormCls_{args.use_norm}_Aug_{args.aug_type}_topK_{args.topk}_wBCE_{args.w_bce}_wEntropy_{args.w_entropy}_featnorm_{args.feat_norm}")
    ...
    if args.mode == 'train':
        ...
        wandb_run_name = f'Direct-Ocra_{args.dataset_name}_S{str(args.current_step)}-{args.num_steps}_{args.model_head}_NormCls_{args.use_norm}_Aug_{args.aug_type}_topK_{args.topk}_wBCE_{args.w_bce}_wEntropy_{args.w_entropy}'
        wandb.init(project='Ours_Ocra',
    ...
    # ----------------------
    model, teachers_list, student, joint_head = build_orca(args)
    ...
    if args.mode == 'train':
        ...
        method = ORCA(model=model, teachers_list=teachers_list, student=student, joint_head=joint_head,
