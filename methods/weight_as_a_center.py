...
class WaaC:
    def __init__(self, model, single_head, learned_single_heads, joint_head, sinkhorn, train_loader, ulb_step_val_list,
    ...
    def train_single(self, args):
        print("=" * 100)
        print(f"\t\t\t\t\tCiao bella! I am WaaC [{1 + args.current_step}/{args.num_steps}] for MSc-iNCD")
        print("=" * 100)
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, ((x_v0, x_v1), _, idx) in enumerate(tqdm(self.train_loader)):
                ...
                self.single_head.normalize_prototypes(s=args.scale)
                ...
                # Single head output
                output_v0 = self.single_head(feat_v0, inference=False)
                output_v1 = self.single_head(feat_v1, inference=False)
