...
from utils.ewc_testers import test_cluster, test_ind_cluster
from utils.ewc_losses import EWCLoss, ZeroPaddingCrossEntropy
...
class EWC:
    ...
    def update_ewc(self, step):
        self.EWC.update(task_id=step,
                        model=self.encoder_shared,
                        head=self.step_single_head_list[step],
                        train_loader=self.step_val_loader_list[step])

    def train(self, args, step=0):
        print("=" * 100)
        print(f"\t\t\t\t\tEWC-Stage-Train [{1+step}/{args.num_steps}] for MSc-iNCD")
        print("=" * 100)
    ...
    def train_warmup(self, args, step=0):
        print("=" * 100)
        print(f"\t\t\t\t\tEWC-Stage-WarmUp [{1+step}/{args.num_steps}] for MSc-iNCD")
        print("=" * 100)
    ...
    def train_incremental(self, args, il_step=1):
        ...
        # generate param list for optimizer
        #   |- encoder
        param_list = list(self.encoder_shared.parameters()) + list(self.step_single_head_list[il_step].parameters())
        ...
        for epoch in range(args.epochs):
            # create loss statistics recorder for each loss
            loss_uno_record = AverageMeter()    # UNO loss recorder
            loss_ewc_record = AverageMeter()    # EWC loss recorder

            # switch the models to train mode
            self.encoder_shared.train()
            self.step_single_head_list[il_step].train()
            ...
            for batch_idx, ((x_v0, x_v1), _, idx) in enumerate(tqdm(self.step_train_loader_list[il_step])):
                ...
                # normalize classifier weights
                with torch.no_grad():
                    weight_temp = self.step_single_head_list[il_step].last_layer.weight.data.clone()
                    weight_temp = F.normalize(weight_temp, dim=1, p=2)
                    self.step_single_head_list[il_step].last_layer.weight.copy_(weight_temp)
                ...
                # EWC loss
                loss_ewc = self.EWC(self.encoder_shared)

                # Total loss
                loss_total = loss_uno + loss_ewc

                loss_uno_record.update(loss_uno.item(), output_v0.size(0))
                loss_ewc_record.update(loss_ewc.item(), output_v0.size(0))
            ...
            wandb.log({
                ...
                f"step-{il_step}/loss/Train_ewc": loss_ewc_record.avg,
            }, step=epoch+args.epochs_warmup)

            print('\n===========================================')
            print('\nTrain Epoch [{}/{}]: Avg Loss: UNO={:.4f},\tEwC={:.4f}'.format(1+epoch,
                                                                                    args.epochs,
                                                                                    loss_uno_record.avg,
                                                                                    loss_ewc_record.avg))
            print('===========================================')
