...
class RankStats:
    ...
    def train_single(self, args):
        print("=" * 100)
        print(f"\t\t\t\t\tCiao bella! I am RankStats Teacher [{1 + args.current_step}/{args.num_steps}] for MSc-iNCD")
        print("=" * 100)
        ...
        for epoch in range(args.epochs):
            ...
            loss_consistency_recorder = AverageMeter()
            ...
            for batch_idx, ((x_v0, x_v1), _, idx) in enumerate(tqdm(self.train_loader)):
                ...
                prob_v0_ulb, _ = PairEnum(prob_v0)
                _, prob_v1_ulb = PairEnum(prob_v1)

                # L_bce: ranking-statistics for single head
                bce_loss = args.w_bce * criterion_bce(prob_v0_ulb, prob_v1_ulb, target_ulb)

                # L_mse: consistency loss for single head
                consistency_loss = w * F.mse_loss(prob_v0, prob_v1)  # + F.mse_loss(prob1, prob1_bar)

                # ce_loss = ce(output[:labeled_len], target)
                if args.w_entropy > 0:
                    entropy_loss = args.w_entropy * entropy(torch.mean(prob_v0, 0))
                else:
                ...
                loss = -entropy_loss + bce_loss + consistency_loss

                loss_bce_recorder.update(bce_loss.item(), x_v0.size(0))
                loss_entropy_recorder.update(entropy_loss.item(), x_v0.size(0))
                loss_consistency_recorder.update(consistency_loss.item(), x_v0.size(0))
            ...
            wandb.log({
                ...
                "loss/consistency": loss_consistency_recorder.avg,
            }, step=epoch)

            print('\n===========================================')
            print('\nTrain Epoch [{}/{}]: Avg Loss: bce={:.4f}, entropy={:.4f}, consistency={:.4f}'.
                  format(1 + epoch, args.epochs, loss_bce_recorder.avg, loss_entropy_recorder.avg,
                         loss_consistency_recorder.avg))
            print('===========================================')
            # save student head
        ...
        print("[Single student head trainign completed]: extended the teacher list by the newly learned student head")
