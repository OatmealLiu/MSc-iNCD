...
from methods.clip_testers import test_cluster, test_ind_cluster, test_ind_cluster_debug

class TeacherStudentClip:
    def __init__(self, model, teachers_list, student, feat_replayer, train_loader, ulb_step_val_list,
    ...
    def train_TeacherStudent(self, args):
        print("=" * 100)
        print(f"\t\t\t\t\tCiao bella! I am Teacher->Student [{1 + args.current_step}/{args.num_steps}] for MSc-iNCD")
        print("=" * 100)
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, ((x_v0, x_v1), _, idx) in enumerate(tqdm(self.train_loader)):
                ...
                # Feature extraction
                feat_v0 = self.model.encode_image(x_v0).float()
                feat_v1 = self.model.encode_image(x_v1).float()
                ...
                target_v0 = output_v0.detach().max(1)[1] + args.current_step * args.num_novel_interval
                target_v1 = output_v1.detach().max(1)[1] + args.current_step * args.num_novel_interval
    ...
    def test_debug(self, args):
        s = args.current_step

        model = self.model
        single_head = self.teachers_list[s]
        joint_head = self.student

        # acc_w_cluster = test_cluster(model, joint_head, self.ulb_step_test_list[s], args)

        acc_wo_cluster = test_ind_cluster_debug(
            model=model, test_head=joint_head, ind_gen_head=single_head,
            test_loader=self.ulb_step_test_list[s], step=s, args=args,
            ind_gen_loader=self.ulb_step_val_list[s])


        print('\n========================================================')
        # print(f'S{s}acc_w_cluster = {acc_w_cluster}')

        print(f'\nS{s}acc_wo_cluster = {acc_wo_cluster}')
        print('========================================================')
