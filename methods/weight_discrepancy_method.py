...
class WeightDiscrepancy:
    ...
    def train_il(self, args):
        ...
        for epoch in range(args.epochs):
            ...
            if args.w_wd == 'damp':
                if args.epoch_wd == 0:
                    w_wd = (1 - epoch/(args.epochs - 1)) * 0.005
                else:
                    if epoch >= args.epoch_wd:
                        w_wd = (1 - (epoch - args.epoch_wd)/(args.epoch_wd - 1)) * 0.005
                    else:
                        w_wd = 1.0
            elif args.w_wd == 'rampup':
                w_wd = args.rampup_coefficient * ramps.sigmoid_rampup(epoch, args.rampup_length) * 0.0002
            else:
            ...
            for batch_idx, ((x_v0, x_v1), _, idx) in enumerate(tqdm(self.train_loader)):
                ...
                if epoch >= args.epoch_wd:
                    ...
                    W_dissim = [W_t.matmul(W_prev_s.T) for W_prev_s in W_prev_list]
                    # +1 to lift the CosSim from [-1,1] to [0,1]
                    W_dissim = [W_ts_dissim + torch.ones_like(W_ts_dissim).float().to(args.device)
                                for W_ts_dissim in W_dissim]

                    # Weight the CosSim matrix by current-data-prev-prototype affinity heatmap
                    if args.conf_guided:
                        W_dissim = [W_dissim[s] * aff_list[s]for s in range(len(W_dissim))]

                    W_dissim = torch.cat(W_dissim, dim=0)
                    loss_weight = torch.sum(W_dissim)
                    loss_weight = (1.0/args.current_step) * w_wd * loss_weight
                else:
