...
from models.build_data_fading import build_data_fading
from data.build_dataset import build_data
from methods.data_fading import DataFading
...
if __name__ == "__main__":
    ...
    # Strategy tricks
    parser.add_argument('--percentage_filter', type=float, default=0.1)
    parser.add_argument('--fading_step', default=0, type=int)
    ...
    model_dir = os.path.join(args.exp_root, f"{runner_name}_{args.model_name}_{args.dataset_name}_Steps{args.num_steps}_FadingStep_{args.fading_step}_FilterPercentage_{args.percentage_filter}")
    ...
    args.log_dir = model_dir + f'/{args.dataset_name}_S{str(args.current_step)}-{args.num_steps}_FadingStep_{args.fading_step}_FilterPercentage_{args.percentage_filter}_log.txt'
    ...
    # WandB setting
    if args.mode == 'train':
        wandb_run_name = f'Data-Fading_{args.model_name}_{args.dataset_name}_S{str(args.current_step)}-{args.num_steps}_FadingStep_{args.fading_step}_FilterPercentage_{args.percentage_filter}'
        wandb_tags = [f'TotalStep={args.num_steps}', args.dataset_name, f'Steps={str(args.current_step)}',
                      args.model_name, f'FadingStep={args.fading_step}', f'FilterPercentage={args.percentage_filter}']
        wandb.init(project='Our_DataFading',
    ...
    # ----------------------
    model, single_head, single_heads_list, joint_head = build_data_fading(args)
    ...
    if args.mode == 'train':
        ...
        method = DataFading(model=model, single_head=single_head, learned_single_heads=single_heads_list,
