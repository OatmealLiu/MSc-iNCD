...
from painter.pencil import Pencil
...
if __name__ == "__main__":
    ...
    # Model Config
    parser.add_argument('--mode', type=str, default='raw', choices=['raw', 'trained_teacher_student'])
    ...
    parser.add_argument('--plot_output_dir', type=str, default='./gallery/')
    ...
    parser.add_argument('--student_head_path', type=str, default='studentHead_S3_cifar100_Steps4_LinearHead_Mlp1.pth')
    ...
    # set a dir name which can describe the experiment
    args.plot_output_dir = os.path.join(args.plot_output_dir, f"{runner_name}_{args.dataset_name}_Steps{args.num_steps}_{args.model_head}_Mlp{args.num_mlp_layers}_Loss{args.student_loss}")
    ...
    if args.mode == 'raw':
        pencil = Pencil(args)
        weight_student = student.last_layer.weight.data
        print(f"Student head weight shape = {weight_student.shape}")
        pencil.plot_weights(weight_student, file_name='l2_student_weight_cifar100.png')
        pencil.plot_logits(model, student, ulb_all_test_loader, file_name='l2_student_logits_cifar100.pdf')
        # Final test with test loader
    elif args.mode == 'trained_teacher_student':
        student_state_dict = torch.load(args.student_head_path, map_location='cuda')
