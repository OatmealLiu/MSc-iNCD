...
class MemoryBank(object):
    def __init__(self, n, dim, num_classes, temperature):
        self.n = n  # len(base_dataset)
        self.dim = dim  # feat_dim

        self.features = torch.FloatTensor(self.n, self.dim)  # extracted features of each sample
        self.targets = torch.LongTensor(self.n)              # GT-label of each sample
        ...
        self.C = num_classes  # how many classes
    ...
    def mine_nearest_neighbors(self, topk=20, calculate_accuracy=False):
    ...
    @torch.no_grad()
    def fill_memory_bank(self, loader, model):
        model.eval()
        self.reset()  # memory.ptr = 0

        for i, batch in enumerate(tqdm(loader)):
            images = batch['image'].cuda(non_blocking=True)
            targets = batch['target'].cuda(non_blocking=True)
            # print(images.shape)
            output = model(images)  # feature extraction
            # print(f"feat.shape={output.shape}, target.shape={targets.shape}")
