...
class FeatureReplayerClip:
    def __init__(self, args, model, teacher_heads_list, data_factory):
        ...
        if args.dataset_name == 'cifar10':              # 6,000 imgs per class
            ...
            # 20 for S5, 8 for S2
            self.num_per_kernel = 10 if args.num_steps >= 5 else 4
        ...
        self.class_mean = torch.zeros(self.num_kernels, 512).cuda()
        self.class_sig = torch.zeros(self.num_kernels, 512).cuda()
        self.class_cov = 0
        self.generate_kernel(args, model, teacher_heads_list, data_factory)

    def generate_kernel(self, args, model, teacher_heads_list, data_factory):
    ...
    def replay_step(self, step):
        ...
        for k in range(step*self.num_interval, (1+step)*self.num_interval):
            if k in self.nan_feat_idx_list:
                # print(f"---> Do not have prototype for label {k}. Pass")
                continue
            k_dist = torch.distributions.Normal(self.class_mean[k], self.class_sig.mean(dim=0))
