...
from models.build_model_ncdil import build_direct_concat_model
from data.build_dataset import build_data
from data.config_dataset import set_dataset_config
from methods.study_ncd_incremental import NCDIL
# from methods.clip_direct_concat_learning import DirectConcatClip
...
if __name__ == "__main__":
    ...
    # Experimental Dir.
    model_dir = os.path.join(args.exp_root, f"{runner_name}_{args.model_name}_{args.dataset_name}_Steps{args.num_steps}_{args.model_head}_Mlp{args.num_mlp_layers}")
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    # Single head weights Dir.
    weights_dir = os.path.join(args.weights_root, f"{args.model_name}_{args.dataset_name}_Steps{args.num_steps}_{args.model_head}_Mlp{args.num_mlp_layers}")
    ...
    args.save_sup_model_path = model_dir + f"/backbone_{args.dataset_name}_Steps{args.num_steps}_{args.model_head}_GradBlocks{args.grad_from_block}.pth"
    ...
    # WandB setting
    if args.mode == 'train':
        wandb_run_name = f'NCDIL-Study_{args.dataset_name}_{args.model_name}_GradBlock-{args.grad_from_block}_S{str(args.current_step)}/{args.num_steps}'
        wandb.init(project='Ours_NCDIL_study',
                   entity=args.wandb_entity,
                   tags=[f'TotalStep={args.num_steps}', args.dataset_name, f'Steps={str(args.current_step)}',
                         f'GradBlock={args.grad_from_block}', args.model_name],
                   name=wandb_run_name,
    ...
    if args.current_step == 0:
        lb_train_loader = data_factory.get_dataloader(split='train', aug='once', shuffle=True,
    ...
    # ----------------------
    model, single_head, single_heads_list, joint_head, temp_single_head = build_direct_concat_model(args)

    if args.mode == 'train':
        # Create Feature Replayer model
        sinkhorn = SinkhornKnopp(args)

        method = NCDIL(model=model,
                       temp_single_head=temp_single_head,
                       single_head=single_head,
                       learned_single_heads=single_heads_list,
                       joint_head=joint_head,
                       sinkhorn=sinkhorn,
                       lb_train_loader=lb_train_loader,
        ...
        # Training
        if args.current_step == 0:
            method.train_supervised_pretrain(args)
            method.save_backbone(path=args.save_sup_model_path)
