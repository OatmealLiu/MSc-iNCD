...
from methods.upper_bound_method import UpperBoundTeacherStudent
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_root', type=str, default='./outputs_upper_frozen/')
    parser.add_argument('--weights_root', type=str, default='./models/single_weights/')

    parser.add_argument('--exp_marker', type=str, default='frozen')
    ...
    model_dir = os.path.join(args.exp_root, f"{runner_name}_{args.dataset_name}_Steps{args.num_steps}_{args.model_head}_Mlp{args.num_mlp_layers}_Loss{args.student_loss}_Block{args.grad_from_block}")
    ...
    # pretrained_teacher_dir = f"{args.weights_root}{args.dataset_name}_Steps{args.num_steps}_{args.model_head}_Mlp{args.num_mlp_layers}"
    ...
    # WandB setting
    if args.mode == 'train':
        wandb_tags = [f'TotalStep={args.num_steps}', "Frozen", args.dataset_name, f'Block={args.grad_from_block}',
                      f'Steps={str(args.current_step)}', args.model_name]
        wandb_run_name = f'Upper_frozen_{args.dataset_name}_S{str(args.current_step)}/{args.num_steps}_{args.model_head}_Mlp{args.num_mlp_layers}_Loss{args.student_loss}_Block{args.grad_from_block}'
        wandb.init(project='Upper_Joint_Train_Frozen',
    ...
    if args.mode == 'train':
        ...
        method = UpperBoundTeacherStudent(model=model, teachers_list=teachers_list, student=student,
        ...
        # Training
        method.train_JointTeacherStudent(args)
