...
from models.build_ewc import build_ewc_model
...
if __name__ == "__main__":
    ...
    # LwF knobs
    parser.add_argument("--w_ewc", default=10000, type=int, help="weight for KD loss")
    parser.add_argument("--alpha_ewc", default=0.5, type=float, help="weight for KD loss")
    ...
    parser.add_argument('--exp_root', type=str, default='./outputs_EwC/')
    ...
    # Experimental Dir.
    model_dir = os.path.join(args.exp_root, f"{runner_name}_{args.model_name}_{args.dataset_name}_Steps{args.num_steps}_Wewc_{args.w_ewc}_Walpha_{args.alpha_ewc}")
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    args.log_dir = model_dir + f'/{runner_name}_{args.model_name}_{args.dataset_name}_Steps{args.num_steps}_Wewc_{args.w_ewc}_Walpha_{args.alpha_ewc}_log.txt'
    ...
    # WandB setting
    # if args.mode == 'train':
    #     wandb_run_name = f'EWC_{args.dataset_name}_{args.model_name}_{args.num_steps}-Steps_Wewc_{args.w_ewc}_Walpha_{args.alpha_ewc}'
    #     wandb.init(project='SOTA_EwC',
    #                entity=args.wandb_entity,
    #                tags=[f'TotalStep={args.num_steps}', args.dataset_name, args.model_name,
    #                      f'W_ewc={args.w_ewc}', f'W_alpha={args.alpha_ewc}', f'device={args.device}'],
    #                name=wandb_run_name,
    ...
    # Model creation
    # ----------------------
    encoder, step_single_head_list, joint_head_container_list = build_ewc_model(args)
    ...
    if args.mode == 'train':
        ...
        method = EWC(
            ...
            joint_head_container_list=joint_head_container_list,
            sinkhorn=sinkhorn,
        ...
        for step_train in range(args.num_steps):
            wandb_run_name = f'EWC_{args.dataset_name}_{args.model_name}_{step_train}/{args.num_steps}-Steps_Wewc_{args.w_ewc}_Walpha_{args.alpha_ewc}'
            step_run = wandb.init(
                project='SOTA_EwC',
                entity=args.wandb_entity,
                tags=[f'TotalStep={args.num_steps}', args.dataset_name, args.model_name,
                      f'W_ewc={args.w_ewc}', f'W_alpha={args.alpha_ewc}', f'device={args.device}',
                      f'CurrentStep={step_train}'],
                name=wandb_run_name,
                mode=args.wandb_mode,
                reinit=True
            )
            ...
            method.train_warmup(args=args, step=step_train)
            #   2. Formal stage: unlock last block of ViT, update both encoder and classifier head
            #           Unlock encoder before training
            method.unlock_encoder(grad_from_block=args.grad_from_block)
            method.train(args=args, step=step_train)
            #   3. Update EWC Fisher information diag-matrix
            method.update_ewc(step=step_train)
            #           Lock encoder after calc EwC Fisher
            method.lock_encoder()
