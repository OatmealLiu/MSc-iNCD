...
from models.build_waac import build_direct_concat_model
from data.build_dataset import build_data
from methods.weight_as_a_center import WaaC
...
if __name__ == "__main__":
    ...
    parser.add_argument('--normalization', type=str, default='cn', choices=['cn', 'pcc'])
    ...
    parser.add_argument('--exp_root', type=str, default='./outputs_waac/')
    parser.add_argument('--weights_root', type=str, default='./models/single_weights_waac/')
    ...
    # Experimental Dir.
    model_dir = os.path.join(args.exp_root, f"{runner_name}_{args.dataset_name}_Steps{args.num_steps}_{args.model_head}_Mlp{args.num_mlp_layers}")
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    # Single head weights Dir.
    weights_dir = os.path.join(args.weights_root, f"{args.dataset_name}_Steps{args.num_steps}_{args.model_head}_Mlp{args.num_mlp_layers}")
    ...
    # WandB setting
    if args.mode == 'train':
        wandb_run_name = f'WaaC_{args.normalization}_Direct-Concat_{args.dataset_name}_S{str(args.current_step)}-{args.num_steps}_{args.model_head}_Mlp{args.num_mlp_layers}'
        wandb.init(project='Ours_WaaC',
                   entity=args.wandb_entity,
                   tags=[f'TotalStep={args.num_steps}', args.dataset_name, f'Steps={str(args.current_step)}',
                         args.model_name, f'Scale={args.scale}', f'Normalization={args.normalization}'],
                   name=wandb_run_name,
    ...
    if args.mode == 'train':
        ...
        method = WaaC(model=model, single_head=single_head, learned_single_heads=single_heads_list,
